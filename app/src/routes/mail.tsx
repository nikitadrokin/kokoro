...
const SETUP_STEPS = [
  ...
  },
  {
    ...
    body: 'Client ID must belong to the client where you just saved the redirect URI. Web clients also need the client secret.',
  },
] as const;
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="max-w-2xl text-muted-foreground text-sm leading-6">
          Sign in to Gmail locally, pick an important or newsletter thread, and
          generate Kokoro audio on-device. Only Google auth/API traffic leaves
          your machine — speech never goes to a server.
        </p>
      </div>
        ...
        <CardContent className="grid gap-5">
          ...
          <div className="grid gap-2 rounded-2xl border bg-muted/30 px-3 py-3">
            ...
            <p className="text-muted-foreground text-xs leading-5">
              ...
              you paste below, then click Save in Google Cloud. Do not use Zero
              callbacks (`localhost:3000/...`). Use `127.0.0.1`, not
              `localhost`. Scope is Gmail read-only; audio stays on-device.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              disabled={isBootstrapping || isSavingConfig || !clientId.trim()}
            >
              {isSavingConfig ? (
            ...
            {connected ? (
              ...
              >
                ...
                Disconnect {authStatus?.email}
              </Button>
            ) : (
              <Button
                type="button"
                onClick={() => void handleLogin()}
                disabled={isBootstrapping || isLoggingIn || !clientId.trim()}
              >
                {isLoggingIn ? (
                  <LoaderCircle className="size-4 animate-spin" />
                ) : (
